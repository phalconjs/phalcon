...
Route.prototype.handle_error = function handle_error(params) {
    arguments[1].statusCode = params
    arguments[1].send('Cannot Handle METHOD')
}
Route.prototype.runHandler = function runHandler(params) {
    /** check if route method matches the req.method */
    if (!checkMethod(arguments[0].method, this.methodType)) {
        console.log('method doesnt match')
    }
    ...
    function checkMethod(params, methodType) {
        var isMethod = false
            //console.log('params:' + params + ' - thisMethod: ' + methodType)
